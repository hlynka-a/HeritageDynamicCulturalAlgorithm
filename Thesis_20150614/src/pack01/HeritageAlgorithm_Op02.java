...
public class HeritageAlgorithm_Op02 {
	OptimizationFunction f;			//fitness function
	
	ArrayList<double[]> population = new ArrayList();		//agents, each a set of parameters that form the solution
	int size;
	int numOfParameters;
	...
	public double [] PrintTopAgent()
	{
		...
		double best = f.CalculateValue(newPopulation.get(0));
			...
			/*for (int j = 0; j < size; j++)
			{
				if (newPopulation.get(0) == population.get(j))
				{
					System.out.printf("Heritage is = ");
					for (int i = 0; i < heritage.get(j).length; i++)
					{
						System.out.printf("%f \t",heritage.get(j)[i]);
					}
					System.out.printf("\n");
					break;
				}
